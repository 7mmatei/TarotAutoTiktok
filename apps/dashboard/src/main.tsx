...
function App() {
  ...
  const diagnosticText=(diagnostic:any):string=>{ if(diagnostic.error)return diagnostic.error; if(diagnostic.eventTypes?.length)return diagnostic.eventTypes.join(", "); const payload=diagnostic.payload && typeof diagnostic.payload === "object" ? diagnostic.payload : undefined; if(payload){const kind=["type","eventType","event","name","action","command"].map((key)=>payload[key]).find((value)=>value!==undefined); if(kind!==undefined)return `unsupported event=${String(kind)}`; const keys=Object.keys(payload).slice(0,8); if(keys.length)return `unsupported payload (keys: ${keys.join(", ")})`;} return "unsupported payload"; };
