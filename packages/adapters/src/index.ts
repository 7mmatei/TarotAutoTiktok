...
export class GeminiReadingGenerator implements ReadingGenerator {
  ...
  async generate(input:{locale:string;question:string;cards:Array<{id:string;orientation:string;meaning:string}>;maxWords:number}):Promise<ReadingOutput> {
    const responseSchema={type:"object",additionalProperties:false,required:["safe","category","opening","cards","summary","closing","spokenText","safetyFlags"],properties:{safe:{type:"boolean",enum:[true]},category:{type:"string"},opening:{type:"string"},cards:{type:"array",minItems:input.cards.length,maxItems:input.cards.length,items:{type:"object",additionalProperties:false,required:["cardId","interpretation"],properties:{cardId:{type:"string"},interpretation:{type:"string"}}}},summary:{type:"string"},closing:{type:"string"},spokenText:{type:"string"},safetyFlags:{type:"array",items:{type:"string"}}}};
    ...
    return readingOutputSchema.parse(JSON.parse(content));
  }
}
...
export type ReadingGeneratorOptions={provider:"deterministic"|"mock"|"gemini";apiKey?:string;model:"gemini-3.5-flash-lite"};
export function createReadingGenerator(options:ReadingGeneratorOptions):ReadingGenerator { const fallback=new DeterministicReadingGenerator(); if(options.provider==="gemini") { if(!options.apiKey) throw new Error("LLM_PROVIDER=gemini requires GEMINI_API_KEY"); return new FallbackReadingGenerator(new GeminiReadingGenerator(options.apiKey,options.model),fallback,2); } return fallback; }
