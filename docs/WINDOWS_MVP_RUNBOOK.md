...
`LLM_PROVIDER=gemini` uses the stable `gemini-3.5-flash-lite` model through Gemini structured output. `GEMINI_API_KEY` is required. Every response is validated, and a Gemini failure is surfaced as a failed request instead of silently substituting a deterministic reading. `/ready` and `/api/status` expose `llm.attempts`, `llm.successes`, `llm.failures`, and the last error so provider usage can be verified. Azure Speech is primary for readings, CTA prompts, and interaction prompts. It retries once and then uses Windows System.Speech locally. CTA audio is synthesized once, cached, randomly selected without an immediate repeat, played only while idle, and stopped before any reading audio begins. Each CTA also rotates Mora through distinct expressions and visual effects.
